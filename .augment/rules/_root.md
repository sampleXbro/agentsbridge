...
2. For every cluster whose triggers match — `file_globs` against the target file path, `command_patterns` (regex) against the Bash command, or `keywords` (case-insensitive substring) against the active task description — read its `file` (e.g. `.agentsmesh/lessons/topics/<topic>.md`).
