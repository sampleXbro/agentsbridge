...
**Amazon Q: ignore and global rules now reach the agent.** Ignore patterns become `toolsSettings.fs_read.deniedPaths` / `fs_write.deniedPaths` in the agent JSON — Q CLI has no ignore file anywhere. Global rules move from `~/.aws/amazonq/rules/` (a directory Q CLI never reads — `paths.rs` `mod global` has no rules constant) into the agent JSON `resources` array, so `rules` at global scope is now honestly `embedded` rather than `native`.
