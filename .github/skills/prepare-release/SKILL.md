...
- **npm package settings → Trusted publishers**: this repository + `../../workflows/publish.yml` must be listed for the `agentsmesh` package so OIDC can publish without `NPM_TOKEN`.
...
- `../../workflows/publish.yml` — runs the changesets release flow on push to `master`.
...
| `../../pull_request_template.md` | type-of-change checklist; **must include a "changeset added" checkbox** alongside TDD + CI |
