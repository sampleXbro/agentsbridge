/**
 * AGENTS.md overlap policy when Codex and compatibility targets emit the same path.
 * Centralizes target-id comparisons outside `src/core/generate`.
 */

import type { CanonicalFiles, GenerateResult } from '../../core/types.js';
import type { ValidatedConfig } from '../../config/core/schema.js';
import { buildReferenceMap } from '../../core/reference/map.js';
import { CODEX_CLI_TARGET_ID } from './target-ids.js';

const AGENTS_SUFFIX = 'AGENTS.md';

function isCodexAgents(result: GenerateResult): boolean {
  return result.target === CODEX_CLI_TARGET_ID && result.path.endsWith(AGENTS_SUFFIX);
}
