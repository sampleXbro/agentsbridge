 ...
 * for MCP servers. Globally, Warp reads `~/.warp/.mcp.json` (same standard
 * `mcpServers` JSON shape). Non-root rules are embedded in the root file.
...
// Global MCP config — `~/.warp/.mcp.json` (standard `mcpServers` JSON shape,
// same format as the project `.mcp.json`). Rebased under the home dir in
// global mode like the other `.warp/` outputs.
