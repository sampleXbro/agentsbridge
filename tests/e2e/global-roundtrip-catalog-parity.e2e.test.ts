...
describe('global round-trip e2e catalog', () => {
  it('every builtin target with globalSupport.layout is listed for round-trip coverage', () => {
    const fromDescriptors = TARGET_IDS.filter((id) => {
      const globalLayout = getBuiltinTargetDefinition(id)?.globalSupport?.layout;
