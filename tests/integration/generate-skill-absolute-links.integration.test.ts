...
describe('generate: skill absolute links → relative (integration, project scope)', () => {
  ...
  it('rewrites POSIX absolute links inside SKILL.md, supporting file, and across skills to destination-relative paths', () => {
    writeFileSync(
...
`,
    ...
    writeFileSync(
      ...
      ['---', 'name: release-manager', 'description: Release manager', '---', 'Release body.'].join(
        '\n',
      ),
    );
  ...
  it('fails generation when a SKILL.md markdown destination contains a non-existent absolute path', () => {
    writeFileSync(
      join(testDir, 'agentsmesh.yaml'),
      `version: 1
targets: [claude-code]
...
describe('generate: skill absolute links → relative (integration, global scope)', () => {
  ...
  it('rewrites HOME-rooted absolute links inside SKILL.md and supporting files to destination-relative paths', () => {
...
`,
  ...
  it('fails global generation when a SKILL.md markdown destination contains a non-existent HOME-rooted absolute path', () => {
    const meshDir = join(homeDir, '.agentsmesh');
