...
describe('agentsmesh lint — cross-target guards', () => {
  ...
  it('warns when canonical permissions exist but a configured target has no permissions support (#1418/1420)', () => {
    // junie: permissions is 'none' at project scope with no target-specific
    // lint.permissions override, so the generic silent-drop guard is the only
    // one that can fire here (cline now has a dedicated partial-support
    // lint.permissions message instead, since it raised to 'partial').
    writeFileSync(
