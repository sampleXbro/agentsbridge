...
import { checkConformance, deriveFingerprint, parseByFormat } from '../../../../src/core/capabilities/fingerprint.js';
import type { LedgerCell } from '../../../../src/core/capabilities/ledger-types.js';

function mcpCell(over: Partial<LedgerCell> = {}): LedgerCell {
  return {
    target: 'cline', feature: 'mcp', scope: 'project', maxAchievable: 'native',
    path: '.cline/mcp.json', ext: '.json', format: 'json',
    fingerprint: {
      topLevelKeys: ['mcpServers'], requiredFrontmatter: [],
      keyChecks: [
...
describe('checkConformance', () => {
  ...
  it('flags a missing top-level key', () => {
    ...
    expect(checkConformance(mcpCell(), '.json', bad).some((i) => /top-level key "mcpServers"/.test(i))).toBe(true);
  });
  it('flags a keyCheck kind mismatch', () => {
    const bad = JSON.stringify({ mcpServers: [] });
    expect(checkConformance(mcpCell(), '.json', bad).some((i) => /\/mcpServers.*object/.test(i))).toBe(true);
  });
  it('flags a missing anyOf child', () => {
    const bad = JSON.stringify({ mcpServers: { fs: { foo: 1 } } });
    expect(checkConformance(mcpCell(), '.json', bad).some((i) => /command.*url/.test(i))).toBe(true);
  });
  ...
  it('flags a yaml parse failure via the shared catch', () => {
    const cell = mcpCell({ ext: '.yaml', format: 'yaml', fingerprint: { topLevelKeys: [], requiredFrontmatter: [], keyChecks: [] } });
  ...
  it('passes an anyOf check whose wildcard iterates an array container', () => {
    const cell = mcpCell({
      fingerprint: {
        topLevelKeys: ['servers'], requiredFrontmatter: [],
