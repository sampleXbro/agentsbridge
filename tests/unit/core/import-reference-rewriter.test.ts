...
describe('createImportReferenceNormalizer', () => {
  ...
  it('normalizes target-specific paths to canonical anchors across common token wrappers', async () => {
    // After the canonical-anchor → colocated-target rewrite contract, the
    // import direction (target-rooted token, canonical destination) projects
    // the reference to the canonical anchor form `.agentsmesh/...` so imported
    // canonical content matches hand-authored canonical content.
    ...
    // Markdown link destinations stay destination-relative; only the link text
    // (a `bracket-label` reading context) and prose tokens take the canonical
    // anchor form.
  ...
  it('normalizes Claude global target prose paths back to canonical paths', async () => {
    ...
    expect(normalized).toBe(
      'Use .agentsmesh/skills/api-gen/references/checklist.md and .agentsmesh/rules/typescript.md.',
    );
  });
  ...
  it('normalizes Antigravity global target prose paths back to canonical paths', async () => {
    ...
    expect(normalized).toBe(
      'Use .agentsmesh/rules/_root.md and .agentsmesh/skills/api-gen/references/checklist.md.',
    );
  });
  ...
  it('normalizes Codex global target prose paths back to canonical paths', async () => {
    ...
    expect(normalized).toBe(
      'Use .agentsmesh/rules/_root.md and .agentsmesh/skills/post-feature-qa/references/edge-case-checklist.md.',
    );
  });
