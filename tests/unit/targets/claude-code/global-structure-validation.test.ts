...
const TEST_DIR = join(tmpdir(), 'am-claude-code-global-structure-test');
...
describe('claude-code global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in home', async () => {
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: 'CLAUDE.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: '.claude.json' });
