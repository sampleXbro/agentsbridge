...
const TEST_DIR = join(tmpdir(), 'am-cline-global-structure-test');
...
describe('cline global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in Documents/Cline/Rules/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, {
        stringContaining: 'Documents/Cline/Rules/typescript.md',
      });
      expect(ruleFile).toBeDefined();
      expect(ruleFile.content).toContain('TypeScript Rules');
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, {
        stringContaining: '.cline/cline_mcp_settings.json',
      });
