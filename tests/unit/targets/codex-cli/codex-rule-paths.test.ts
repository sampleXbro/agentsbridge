...
describe('codexAdvisoryInstructionPath', () => {
  it('uses glob directory prefix for path', () => {
    expect(
      codexAdvisoryInstructionPath(rule('/p/.agentsbridge/rules/typescript.md', ['src/**/*.ts'])),
    ).toBe('src/AGENTS.md');
  });

  it('uses slug for **/… globs', () => {
    expect(
      codexAdvisoryInstructionPath(rule('/p/.agentsbridge/rules/typescript.md', ['**/*.ts'])),
  ...
  it('uses AGENTS.override.md when codex_instruction is override', () => {
    expect(
      codexAdvisoryInstructionPath(
        rule('/p/.agentsbridge/rules/payments.md', ['services/payments/**'], { override: true }),
      ),
  ...
  it('falls back to slug for brace-prefixed ambiguous globs', () => {
    expect(
      codexAdvisoryInstructionPath(
        rule('/p/.agentsbridge/rules/typescript.md', ['{src,tests}/**/*.ts']),
      ),
    ).toBe('typescript/AGENTS.md');
  });
});
