...
const TEST_DIR = join(tmpdir(), 'am-codex-cli-global-structure-test');
...
describe('codex-cli global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .codex/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.codex/AGENTS.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: '.codex/config.toml' });
