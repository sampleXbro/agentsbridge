...
const TEST_DIR = join(tmpdir(), 'am-continue-global-structure-test');
...
describe('continue global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .continue/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.continue/rules/general.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, {
        stringContaining: '.continue/mcpServers/agentsmesh.json',
      });
