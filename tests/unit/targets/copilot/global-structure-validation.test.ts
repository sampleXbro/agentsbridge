...
const TEST_DIR = join(tmpdir(), 'am-copilot-global-structure-test');
...
describe('copilot global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .copilot/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, {
        stringContaining: '.copilot/copilot-instructions.md',
      });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      // Copilot doesn't generate MCP in global mode
