...
const TEST_DIR = join(tmpdir(), 'am-gemini-cli-global-structure-test');
...
describe('gemini-cli global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .gemini/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.gemini/GEMINI.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: '.gemini/settings.json' });
