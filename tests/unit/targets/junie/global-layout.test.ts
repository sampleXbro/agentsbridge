...
describe('junie global frontmatter preservation', () => {
  const TEST_DIR = join(tmpdir(), 'am-junie-global-fm');
  ...
  it('preserves skill frontmatter in global mode', async () => {
    ...
    const skill = results.find(
      (r) => r.target === 'junie' && r.path === `${JUNIE_GLOBAL_SKILLS_DIR}/debugging/SKILL.md`,
    );
  ...
  it('embeds rule content in root instructions in global mode', async () => {
    ...
    const rootFile = results.find((r) => r.target === 'junie' && r.path === JUNIE_GLOBAL_AGENTS_MD);
  ...
  it('preserves MCP configuration in global mode', async () => {
    ...
    const mcpFile = results.find((r) => r.target === 'junie' && r.path === JUNIE_GLOBAL_MCP_FILE);
