...
const TEST_DIR = join(tmpdir(), 'am-junie-global-structure-test');
...
describe('junie global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .junie/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.junie/AGENTS.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: '.junie/mcp/mcp.json' });
