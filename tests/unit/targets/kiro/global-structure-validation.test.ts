...
const TEST_DIR = join(tmpdir(), 'am-kiro-global-structure-test');
...
describe('kiro global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .kiro/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.kiro/steering/AGENTS.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: '.kiro/settings/mcp.json' });
