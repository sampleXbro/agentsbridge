...
const TEST_DIR = join(tmpdir(), 'am-roo-code-global-structure-test');
...
describe('roo-code global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in .roo-code/', async () => {
      setupTestDir();
      ...
      const ruleFile = findGeneratedFile(files, { stringContaining: '.roo/AGENTS.md' });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, { stringContaining: 'mcp_settings.json' });
