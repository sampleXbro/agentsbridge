...
describe('Roo Code structure validation', () => {
  describe('generateRules', () => {
    it('generates ROO.md for root rule', () => {
      ...
      const rooMd = files.find((f) => f.path === '.roo/rules/00-root.md');
      expect(rooMd).toBeDefined();
      validateNoCanonicalPaths(rooMd!.content);
    });

    it('generates .roo/rules/*.md for non-root rules', () => {
      const canonical = makeCanonical({
        rules: [
          {
      ...
      const files = generateRules(canonical);
      const ruleFile = files.find((f) => f.path.startsWith('.roo/rules/'));
