...
const TEST_DIR = join(tmpdir(), 'am-windsurf-global-structure-test');
...
describe('windsurf global mode structure validation', () => {
  describe('Global rules generation', () => {
    it('generates rules in home', async () => {
      ...
      const ruleFile = findGeneratedFile(files, {
        stringContaining: '.codeium/windsurf/memories/global_rules.md',
      });
  ...
  describe('Global MCP generation', () => {
    it('generates MCP config', async () => {
      ...
      const mcpFile = findGeneratedFile(files, {
        stringContaining: '.codeium/windsurf/mcp_config.json',
      });
