...
`init --lessons` adds three more — `.agentsmesh/lessons/recall-log.jsonl`, `capture-log.jsonl` and `outcome-log.jsonl` — because those logs are per-machine telemetry, not shared config.
