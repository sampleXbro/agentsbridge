...
| `--lessons` | Scaffold the project-level lessons recall + capture subsystem and append its procedural rule to `_root.md`. Cannot be combined with `--global`. |
...
`--lessons` creates `.agentsmesh/lessons/index.yaml`,
`.agentsmesh/lessons/journal.md`, and `.agentsmesh/lessons/topics/`, then
appends a tool-agnostic recall/capture protocol to `.agentsmesh/rules/_root.md`.
After `generate`, every enabled target receives the same root rule: agents read
the index, load only matching topic files before edits or commands, and append
failures to the journal.
