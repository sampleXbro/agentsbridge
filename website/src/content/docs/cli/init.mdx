...
1. **Import** — if existing tool configs are detected (Cursor, Claude Code, …), asks once whether to import them all into `.agentsmesh/`.
2. **Targets** — multi-select which tools to generate for. Nothing is pre-selected, so you must pick at least one; the recommended tools are listed first.
3. **Lessons** — enable the shared recall/capture memory (default **yes**).
4. **Generate** — optionally run `generate` right away.

Nothing is written until the end, so **Ctrl-C cancels cleanly** — no partial files.
...
<Aside type="note">
  If the project is already initialized (`agentsmesh.yaml` exists), `init` stops with an "already initialized" error instead of overwriting anything. Use `agentsmesh init --lessons` to add lessons to an existing project, or remove `agentsmesh.yaml` to re-init. See [Already initialized](#already-initialized).
</Aside>
