...
Additional hardening: `git+file://` sources are blocked by default to prevent local privilege escalation on shared hosts — set `AGENTSMESH_ALLOW_LOCAL_GIT=1` to enable them for closed-network development. Skill supporting-file traversal does not follow symlinks, and credentials are redacted from all error output. See [Fetch hardening](../../configuration/extends/#fetch-hardening) for the full list.
