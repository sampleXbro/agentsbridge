...
Triggers may be shared across lessons in a topic, so a match produces a broad **candidate** set. A query therefore **ranks** candidates (BM25 over rule text fused with trigger specificity) and returns the **top 10 by default** — matching is "this lesson is a candidate", not "you must read all of these". A stderr notice reports how many matched; use `--top <n>`/`--all`/`--max-tokens` to widen. `validate` emits a `HIGH_FANOUT_TRIGGERS` warning when triggers are over-shared; per-lesson trigger refinement is the remaining precision lever.
:::
...
| `--all` | Return every match (disable the default cap). |
| `--max-tokens <n>` | Cap results by cumulative estimated rule-token cost. |

Results are **relevance-ranked** (BM25 over rule text fused with trigger specificity) and capped to the top 10 by default; a truncation notice on stderr reports how many matched. Pass `--all` or a larger `--top` to see the rest.
...
| `--trigger-cmd <regex>` | `command_pattern` trigger. Opaque — repeat for multiple; must be a valid regex (else capture is rejected). |
