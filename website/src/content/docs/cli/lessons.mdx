...
| `--trigger-cmd <regex>` | `command_pattern` trigger. Opaque — repeat for multiple; must be a valid regex within the **provably-linear subset** (no quantified groups `(…)+`, adjacent repetition `a+a+`, backreferences, or lookarounds). Anything else is rejected at capture — recall executes these on every command, so it fails closed against ReDoS. |
