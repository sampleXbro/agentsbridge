...
| `--trigger-cmd <regex>` | `command_pattern` trigger. Opaque — repeat for multiple; must be a valid **and** ReDoS-safe regex (an invalid one, or a catastrophic-backtracking one like `(a+)+`, is rejected at capture). |
