...
- **Parallel captures on separate branches can conflict** at merge time (both edit the same JSON tables). The deterministic serialization keeps a manual resolution tractable, and `agentsmesh lessons validate` (run in CI via `agentsmesh lint`) catches a botched resolution before it ships. A union merge driver is on the roadmap.
