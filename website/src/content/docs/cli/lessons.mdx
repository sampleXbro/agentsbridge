...
**`agentsmesh init --lessons` wires this automatically.** It injects a recall hook under **both** `PreToolUse` (guards the **first touch** — recall injects before the edit) and `PostToolUse` (covers later actions and harnesses that support only post-call injection) into `.agentsmesh/hooks.yaml`, so `generate` projects them to every hook-capable target (Claude Code, Cursor, Copilot, …). Targets with no hook support simply keep the always-on lessons paragraph in their root instruction as the universal fallback. The injected entries:
...
`agentsmesh lessons hook` reads the harness's hook payload from stdin, **echoes its `hook_event_name`** so the same command serves as a `PreToolUse` or `PostToolUse` hook, recalls lessons for the touched `file_path` / `command`, and emits the harness context-injection JSON (`hookSpecificOutput.additionalContext`). It uses the harness `session_id` for [dedup](#query), so a lesson is injected at most once per session: the `PreToolUse` hook surfaces it before the first touch, and the `PostToolUse` hook's later runs de-duplicate to nothing.

It is **safe everywhere**: the command is harness-adaptive and a silent no-op (exit 0, no output) on any payload it doesn't recognize, so projecting the hook to a target whose hooks can't inject context (or to a non-hook tool call) does nothing rather than breaking the run.

**Why both `PreToolUse` and `PostToolUse`?** `PreToolUse` fires before the tool runs and **can inject context** via `hookSpecificOutput.additionalContext`, so it guards the **first touch** of a file — the matching lessons reach the model before the edit, with no blocking and no permission interrupt. `PostToolUse` is kept as the fallback for harnesses that support post-call context injection but not pre-call. Capture, by contrast, stays model-driven: a hook can't judge "this was a failure worth a lesson" or author the rule — it can only run, not reason — so the agent still issues `lessons add` itself.
