...
When `.agentsmesh/lessons/` is present, lint validates:

- `index.yaml` parses as valid YAML matching the lessons index schema
- Every topic file referenced in the index exists under `topics/`
- `journal.md` is present and well-formed

These checks run only when the lessons subsystem has been scaffolded (`agentsmesh init --lessons`). Projects without lessons are unaffected.
