...
| `lock_features` | `string[]` | Features protected from accidental regeneration. Only enforced when `strategy` is `lock`. |
...
`lock_features` protects specific features from being silently overwritten during `generate`. **It is only enforced when `strategy: lock` is set** — under the default `merge` strategy the list is accepted but has no effect on `generate`.
