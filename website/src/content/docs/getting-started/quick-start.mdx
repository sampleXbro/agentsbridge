...
1. **Scaffold the canonical directory**
   ...
   <Aside type="tip">
     On an interactive terminal, `agentsmesh init` asks which tools to target and whether to enable lessons. In CI or with `--yes` it runs non-interactively with sensible defaults.
   </Aside>
...
1. **Initialize**
   ...
   On a terminal this launches a short [wizard](../../cli/init/#interactive-wizard) — it scans for existing configs from every [supported AI coding tool](../../reference/supported-tools/) and offers to import them, lets you pick targets, and (in a project) toggles lessons.
