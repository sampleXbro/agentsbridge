...
| Field | Purpose |
|-------|---------|
| `managedOutputs` | `dirs` and `files` AgentsMesh fully manages — stale artifacts are deleted on `generate`. Put a shared config file the user also owns in `coOwnedFiles` instead: it is never deleted, and your `mergeGeneratedOutputContent` hook keeps their content. A path claimed by that hook must not appear in `files`. |
...
Because a co-owned file is never deleted, emptying a canonical source is the only
way a user can revoke what a previous run wrote into it. Your generator cannot do
that — it sees canonical alone, so it returns nothing and the path drops out of
the run entirely. Declare a `revocableOutputs` claim instead:

```js
revocableOutputs(scope) {
  return [
    {
      feature: 'mcp',                        // 'mcp' | 'permissions' | 'hooks'
      path: '.foo-ide/settings.json',        // must be a coOwnedFiles path
      cleared: '{"mcpServers":{}}',          // projection, not a finished file
      holds: (existing) => {                 // still holds something of ours?
        try {
          return Object.keys(JSON.parse(existing).mcpServers ?? {}).length > 0;
        } catch {
...
`cleared` is a projection: AgentsMesh folds it into the file with your own
`mergeGeneratedOutputContent` hook, so keys you leave out are untouched. Write it
in whatever shape that hook reads as "this key is mine and it is empty now".

A claim fires only when the feature is enabled AND the canonical source exists
but lists nothing. A missing source (no `.agentsmesh/mcp.json` at all) and a
disabled feature both mean "no opinion" — AgentsMesh leaves the user's keys
alone. AgentsMesh also stands down when the file does not exist, and when `holds`
returns `false`, so revocation never creates a config the tool never had and
never reflows a file that carries nothing of yours.

Only `mcp`, `permissions` and `hooks` can drive a claim. Rules, commands, agents,
skills and ignore come from directories, where "empty" and "missing" are
indistinguishable, so they never revoke.
...
| `revocableOutputs` | `function` | Cleared-key claims for `coOwnedFiles` paths (see above) |
