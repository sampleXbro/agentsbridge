...
- `--cmd <c>` matches `command_pattern` triggers. Mandatory recall executes these on every command, and a backtracking `RegExp` cannot be proven linear by inspection (even `a+b` is quadratic on a long non-matching input). So command patterns are matched by an in-repo **non-backtracking engine** (a Thompson NFA): matching is linear in the input length for any pattern it can compile, so no pattern — `(a+)+`, `a+a+`, or a 30k-char adversarial command — can hang recall. The engine supports literals, escapes (`\d \w \s`, `\xHH`, `\uHHHH`, control chars), `.`, char classes, anchors (`^ $ \b`), groups, `|`, and quantifiers (`* + ?`, `{n,m}`) — with `new RegExp` semantics (no `u` flag), so `\x61`/`a` decode to `a` and `[\b]` is a backspace. Patterns it cannot evaluate — invalid syntax, **backreferences**, **lookarounds**, or ones that **expand to too large an NFA** (e.g. `a{1000}` ×10, since match cost is O(states × input), not O(pattern length)) — are flagged `UNSAFE_TRIGGER_PATTERN` (or `INVALID_TRIGGER_PATTERN`), rejected at capture, and skipped at read time (fail closed). Matching is iterative (no recursion) and the input length is bounded, so neither a deep pattern nor a huge command can exhaust the stack or the time budget. See `isSafeRegexPattern` in the programmatic API.
...
| `UNSAFE_TRIGGER_PATTERN` | error | A `command_pattern` the linear matcher cannot evaluate — a backreference or lookaround (features a non-backtracking engine cannot run). Backtracking-prone shapes like `(a+)+` are NOT flagged: the engine runs them in linear time. Rejected at capture, skipped at read time. |
