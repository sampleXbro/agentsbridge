...
description: AgentsMesh exposes itself as an MCP server so AI agents can introspect and mutate their own canonical configuration — rules, commands, agents, skills, settings, and orchestration — all over stdio.
---
...
- **Resource limits** — individual file writes are capped to prevent runaway mutations.
- **No absolute paths in error messages** — `McpError` refuses to leak filesystem paths to the agent.

<Aside type="caution">
  The server operates on your project's canonical directory. Any agent with access to this MCP server can modify rules, agents, skills, and trigger generation. Grant access only to trusted agents.
</Aside>
