...
| `agentsmesh/lessons` | Lesson recall/capture helpers: scaffold lessons, load the trigger index, read triggered topic files, append capture bullets, and reuse distill primitives. |
...
Creates `.agentsmesh/lessons/` and appends the tool-agnostic lessons protocol to
`.agentsmesh/rules/_root.md`. This is the programmatic equivalent of
...
const event: ToolEvent = { kind: 'bash', command: "rg 'foo' src" };
const lessons = readTriggeredLessons(process.cwd(), event);

appendLessonToJournal(process.cwd(), {
  heading: 'Search pattern parsed as a flag',
  whatWentWrong: 'ripgrep rejected the command before searching',
  rootCause: 'the pattern started with a dash',
  rule: 'pass dash-leading patterns with -e',
});
```

`readTriggeredLessons(projectRoot, event)` loads the single
`.agentsmesh/lessons/index.yaml`, matches the generic tool event (`edit`,
`write`, `bash`, or `task`), and reads only the matched topic files. The API has
no target parameter because lessons are intentionally shared across every
supported tool through the generated root rule.

Lower-level helpers (`parseIndex`, `matchTriggers`, `parseBullets`,
`hashBullet`, `loadLedger`, `saveLedger`, and `scoreBullet`) remain available
for custom distillers or IDE integrations.
