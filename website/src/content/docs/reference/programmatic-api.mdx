...
| `agentsmesh/lessons` | Lessons recall/capture graph: load/save `lessons.json`, `queryLessons` for recall, `addLesson` for capture, `validateLessonsGraph`, `importLegacyLessons` (one-shot upgrade migrator), and `scaffoldLessons`. |
...
`queryLessons(graph, { file, command, keyword })` returns every `active` lesson
whose triggers match any supplied predicate. The combination is OR across
triggers — see the [recall semantics](../lessons/#recall-semantics) for the full
contract.
