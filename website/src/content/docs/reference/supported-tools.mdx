...
Kiro IDE is strongest as a split project surface: root instructions live in `AGENTS.md`, additional rules live in `.kiro/steering/`, skills stay native, hooks use `.kiro/hooks/*.kiro.hook`, MCP uses `.kiro/settings/mcp.json`, and ignore patterns use `.kiroignore`.

| Feature | Format | Notes |
|---------|--------|-------|
| Rules | `AGENTS.md`, `.kiro/steering/*.md` | Root guidance in `AGENTS.md`; contextual rules use steering frontmatter |
| Commands | — | No documented Kiro IDE repo-local commands directory |
| Agents | — | Kiro IDE agent files are not generated by default |
| Skills | `.kiro/skills/*/SKILL.md` | Native Agent Skills format |
| MCP Servers | `.kiro/settings/mcp.json` | Workspace MCP config |
| Hooks | `.kiro/hooks/*.kiro.hook` | Native JSON hook files |
| Ignore | `.kiroignore` | gitignore syntax |
